...
class ParticleEmitter extends Entity {
    constructor(particleCount, playArea, scene) {
        super();
        ...
        var uniforms = {
            ...
            texture: {
                type: "t",
                value: THREE.ImageUtils.loadTexture("assets/images/Laser.png"),
            },
        };
    ...
    createExplosion(position, qty, force, forceV) {
        force = force || 20;
        forceV = forceV || 20;
        var count = qty;
        while (--count) {
    ...
    createBurst(position, direction, qty, force, forceV, spreadAmount) {
        force = force || 20;
        forceV = forceV || 20;
        var count = qty;
        while (count--) {
    ...
    update(dt) {
        ...
        for (var i = 0, i3 = 0; i < this.particleCount; i++, i3 += 3) {
            ...
            tmpVector.set(0, 0, 0).add(particle.velocity).multiplyScalar(dt);
            ...
            if (
                particle.position.x <= this.playArea.left ||
                particle.position.x >= this.playArea.right
            ) {
            ...
            if (
                particle.position.y <= this.playArea.bottom ||
                particle.position.y >= this.playArea.top
            ) {
            ...
            particle.rotation =
                Math.atan2(tmpVector.x, tmpVector.y) + Math.PI / 2;
